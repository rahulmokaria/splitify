...
const gettransaction=async (req,res)=>{
    try {
        const {userid}=req.body;
        var result=await transactionmodel.find({userid});
        if(!result) return helper(res,"No transaction till now",200,true);
        const temp=result.sort((p1,p2)=>p1.date.toDateString()>p2.date.toDateString()?-1:1)
        console.log(temp)
        return helper(res,temp,200,true);
    } catch (error) {
        return helper(res,error.message,400,false);
    }
}
module.exports={registercontroller,logincontroller,userdetailscontroller,addtransaction,getpiechart,gettransaction}